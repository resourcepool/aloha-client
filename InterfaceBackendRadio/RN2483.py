...
class RN2483(object):
    def __init__(self, port):
        ...
        self.reset()
        print(self.command('sys get ver').decode("utf-8"))

    def command(self, cmdIn, check=False):
        self._ser.write(str.encode(cmdIn+'\r\n'))
        result = self._ser.readline().splitlines()[0]
        if check and result.decode("utf-8") != 'ok':
            raise Exception('Cmd "%s" responded with "%s"'%(cmdIn, result.decode("utf-8")))
        return result
    ...
    def transmit(self, s):
        ...
        print(out)
        out = hexlify(str.encode(s)).decode("utf-8")
        print(out)
        return self.command('radio tx %s'%out) == 'ok'

    def receive(self):
        result = self.command("mac pause")
...
if __name__ == '__main__':
    ...
    parser.add_option("--rx", action="store_true", dest="rx", help="Receive messages")
